        ...
        <View style={{ backgroundColor: '#fff', flex: 1 }}>
            ...
            <ScrollView>
                <View style={styles.news2}>
                    {sportsNews?.length > 0 && sportsNews.map((item, index) => (
                        <View key={index} style={[styles.news2Child2, styles.childLayout1, { backgroundColor: (index % 2) === 0 ? '#fff3dc' : '#fff' }]} >
                            <View style={{ borderTopEndRadius: 16, marginLeft: 16, marginRight: 16 }}>
                                ...
                                <Text style={[styles.centerReceivesSeven8, styles.centerTypo2]}>{item?.title}</Text>
                                <Pressable style={styles.centerRecievesForContainer} onPress={() => navigation.navigate('ViewFullDetails', { item: item })}>
                                    <Text style={styles.textTypo2}>{item?.description}
